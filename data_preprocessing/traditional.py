...
class ColorTransfer:
    ...
    def lab_transfer(self, img_arr_in=None, mask=None):
        ...
        input = img_arr_in.reshape(-1, c).transpose()
        mask_flat = mask.reshape(-1, c).transpose() / 255.0
        idx_1 = np.argwhere(np.all(mask_flat[..., :] == 1, axis=0))
        lab_ref = np.delete(input, idx_1, axis=1).transpose()
        lab_in = input.transpose()
        
        lab_in = np.expand_dims(lab_in, axis=1)
        lab_ref = np.expand_dims(lab_ref, axis=1)

        lab_in = cv2.cvtColor(lab_in, cv2.COLOR_BGR2LAB)
        lab_ref = cv2.cvtColor(lab_ref, cv2.COLOR_BGR2LAB)

        lab_out = self.mean_std_transfer(img_arr_in=lab_in,
                                         img_arr_ref=lab_ref)

        img_arr_out = cv2.cvtColor(lab_out, cv2.COLOR_LAB2BGR)
        img_arr_out = img_arr_out.reshape(h, w, c)
        mask = mask / 255.0
        img_arr_out = img_arr_in * (1-mask) + img_arr_out * mask
        return img_arr_out
    ...
    def pdf_transfer(self, img_arr_in=None, mask=None, regrain=False):
        ...
        [h, w, c] = img_arr_in.shape
        input = img_arr_in.reshape(-1, c).transpose() / 255.0
        mask_flat = mask.reshape(-1, c).transpose() / 255.0
        idx_1 = np.argwhere(np.all(mask_flat[...] == 1, axis=0))
        reshape_arr_ref = np.delete(input, idx_1, axis=1)
        reshape_arr_in = input
...
pdf_path = '{}/pdf'.format(result_path)
if not os.path.exists(pdf_path):
    os.makedirs(pdf_path)
    
lab_path = '{}/lab'.format(result_path)
if not os.path.exists(lab_path):
    os.makedirs(lab_path)
    
input_path = '../video0/test/composed_image'
mask_input_path = '../video0/test/mask'

count = 0
for data in os.listdir(input_path):
    img_path = os.path.join(input_path, data)

    img = cv2.imread(img_path, cv2.IMREAD_UNCHANGED)
    input = img[:,:,:3]
    
    mask_path = os.path.join(mask_input_path, data.replace('composed', 'mask'))
    mask = cv2.imread(mask_path)
    # mask = np.repeat(np.expand_dims(mask, axis = 2), 3, axis = 2)
    cv2.imwrite('{}/mask/{}'.format(result_path, data.replace('composed', 'mask')), mask)
    PT = ColorTransfer()
    img_trans = PT.pdf_transfer(img_arr_in=input,
                                mask = mask,
                                regrain=True)
    cv2.imwrite('{}/{}'.format(pdf_path, data.replace('composed', 'pdf')), img_trans)
    img_trans = PT.lab_transfer(img_arr_in=input, mask=mask)
    cv2.imwrite('{}/{}'.format(lab_path, data.replace('composed', 'lab')), img_trans)
    
    print('Image{} generated'.format(count))
    count += 1