...
from attention import SpatialSeparatedAttention
...
class UNetDecoder(nn.Module):
    def __init__(self):
        ...
        attention_layer = SpatialSeparatedAttention

        block0 = UNetUpBlock(in_channels=512, out_channels=256, attention_layer=None, activation=nn.ReLU, padding=1)
        block1 = UNetUpBlock(in_channels=256, out_channels=256, attention_layer=None, activation=nn.ReLU, padding=1)
     
        block2 = UNetUpBlock(in_channels=256, out_channels=128, attention_layer=None, activation=nn.ReLU, padding=1)
        block3 = UNetUpBlock(in_channels=128, out_channels=128, attention_layer=attention_layer, activation=nn.ReLU, padding=1)
    
        block4 = UNetUpBlock(in_channels=128, out_channels=64, attention_layer=attention_layer, activation=nn.ReLU, padding=1)
        block5 = UNetUpBlock(in_channels=64, out_channels=64, attention_layer=attention_layer, activation=nn.ReLU, padding=1)
    ...
    def forward(self, encoder_outputs, mask):
        ...
        for up_block, skip_output in zip(self.up_blocks, encoder_outputs[1:]):
            output = up_block(output, skip_output, mask)
...
class UNetUpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, attention_layer, activation, padding):
        ...
        if attention_layer is not None:
            self.attention = attention_layer(out_channels, activation=activation)
        else:
            self.attention = None

    def forward(self, x, encoder_output, mask):
        output = self.deconv(x) + encoder_output
